...
interface Props { cards: CreditCard[]; onRemove: (id: string) => void; scores?: Record<string, number> }
...
const ROWS: Row[] = [
  ...
  { label: 'Cashback', render: (c) => value(c.cashbackCategories) },
  { label: 'Lounge access', render: (c) => value([c.loungeDomestic, c.loungeInternational].filter(Boolean).join('; ')) },
  { label: 'Insurance', render: (c) => value(c.insurance) },
  { label: 'Fuel benefits', render: (c) => value(c.fuel) },
  { label: 'Dining benefits', render: (c) => value(c.dining) },
  { label: 'Shopping benefits', render: (c) => value(c.shopping) },
  { label: 'Travel benefits', render: (c) => value(c.travel) },
  { label: 'Forex benefits', render: (c) => value(c.forex) },
  { label: 'UPI benefits', render: (c) => value(c.upi) },
  ...
  { label: 'Pros', render: (c) => value(c.pros.join(', ')) },
  { label: 'Cons', render: (c) => value(c.cons.join(', ')) },
];
...
export function ComparisonTable({ cards, onRemove, scores = {} }: Props) {
  return (
    <div className="overflow-x-auto rounded-xl border border-border">
      <table className="w-full min-w-[640px] border-collapse text-sm">
        ...
        <tbody>{[...ROWS, { label: 'Overall recommendation score', render: (card: CreditCard) => scores[card.id] === undefined ? <NotAvailable /> : scores[card.id] }].map((row, index) => <tr key={row.label} className={index % 2 ? 'bg-secondary/30' : undefined}>
